// The X11 backend reads its keysym tables from waylandz's XKB code and z11 speaks Unix sockets, so like the Wayland backend it exists only on Linux; a Windows build must not analyze it.
